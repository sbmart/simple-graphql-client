  ...
  function UrqlClient() {
    ...
    _this.update = function (changedTypes, response, refresh) {
      ...
      if (_this.props.shouldInvalidate) {
        invalidated = _this.props.shouldInvalidate(changedTypes, _this.typeNames, response, _this.state.data);
    ...
    _this.fetch = function (opts, initial) {
      ...
      if (!Array.isArray(_this.query)) {
        ...
        client.executeQuery(_this.query, skipCache).then(function (result) {
          // Store the typenames
          if (result.typeNames) {
        ...
        var partialData = [];
        return Promise.all(_this.query.map(function (query) {
