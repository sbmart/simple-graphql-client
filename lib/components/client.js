      ...
      _this.invalidate = function(queryObject) {
        ...
        if (queryObject) {
          var stringified = JSON.stringify(
            (0, _typenames.formatTypeNames)(queryObject)
          );
            ...
            ? Promise.all(
                _this.props.query.map(function(q) {
                  return cache.invalidate(
                    (0, _hash.hashString)(JSON.stringify(q))
                  );
                })
              )
            : cache.invalidate(
                (0, _hash.hashString)(JSON.stringify(_this.query))
              );
        }
      };
      ...
      _this.formatProps = function(props) {
        ...
        if (props.query) {
          ...
          _this.query = Array.isArray(props.query)
            ? props.query.map(_typenames.formatTypeNames)
            : (0, _typenames.formatTypeNames)(props.query); // Subscribe to change listener
        ...
        if (props.mutation) {
          ...
          Object.keys(props.mutation).forEach(function(key) {
            _this.mutations[key] = (0, _typenames.formatTypeNames)(
              props.mutation[key]
            );
          }); // bind to mutate
